                               ...
                               LedBlinkUtility_Colors color)
{
    ...
    if (ledIndex >= openedLeds) {
        ...
        TimerUtility_TimerAdd(&rise, &blinkOrdinaryTime,
...
int LedBlinkUtility_BlinkLeds(RgbLed **leds, size_t ledCount)
{
    ...
    for (size_t i = 0; i < ledCount; i++) {
        ...
        if (TimerUtility_TimerEqual(&ledRisingFallingEdgeIntervalTime[i].falling, &invalidTime) ||
            (TimerUtility_TimerEqual(&ledRisingFallingEdgeIntervalTime[i].rising, &invalidTime)))
            continue;

        bool turnedOn = (TimerUtility_TimerCompareLesserEqual(
                             &currentTime, &ledRisingFallingEdgeIntervalTime[i].falling) &&
                         TimerUtility_TimerCompareGreater(
    ...
    if (TimerUtility_TimerCompareLesserEqual(&blinkingLedNextBlinkTime, &currentTime)) {
        TimerUtility_TimerAdd(&currentTime, &periodicBlinkingLedPeriod, &blinkingLedNextBlinkTime);
